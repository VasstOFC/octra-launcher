...
import type { ContentItem, ContentOwner } from '@lumen/ui'
...
export async function is_file_on_Lumen(projectPath: string): Promise<boolean> {
	return await invoke('plugin:instance|instance_is_file_on_lumen', { projectPath })
}
...
// Update a managed Lumen instance to a specific version
export async function update_managed_Lumen_version(
	instanceId: string,
	versionId: string,
): Promise<InstallJobSnapshot> {
	return await invoke('plugin:instance|instance_update_managed_lumen_version', {
		instanceId,
		versionId,
	})
}

// Repair a managed Lumen instance
export async function update_repair_Lumen(instanceId: string): Promise<InstallJobSnapshot> {
	return await invoke('plugin:instance|instance_repair_managed_lumen', { instanceId })
}
