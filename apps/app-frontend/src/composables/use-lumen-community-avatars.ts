...
export const Lumen_COMMUNITY_STEVE_HEAD =
...
export function useLumenCommunityAvatars(members: Ref<MemberLike[]>) {
	...
	function avatarFor(member: MemberLike): string {
		void revision.value
		return headByKey.value.get(memberKey(member)) ?? Lumen_COMMUNITY_STEVE_HEAD
	}
	...
	async function resolveMember(member: MemberLike): Promise<void> {
		...
		if (cached && cached !== Lumen_COMMUNITY_STEVE_HEAD) {
			return
		}
		if (cached === Lumen_COMMUNITY_STEVE_HEAD) {
		...
		try {
			for (const texture of candidateSkinUrls(member)) {
				try {
					const skin: Skin = {
						texture_key: `Lumen-community-${key}`,
						variant: 'CLASSIC',
						texture,
			...
			headByKey.value = new Map(headByKey.value).set(key, Lumen_COMMUNITY_STEVE_HEAD)
	...
	return {
		avatarFor,
		steveHead: Lumen_COMMUNITY_STEVE_HEAD,
	}
}
