...
import {
	Lumen_COMMUNITY_STEVE_HEAD,
	useLumenCommunityAvatars,
} from '@/composables/use-lumen-community-avatars'
import {
...
import {
	...
	LumenCacheMrpackUrl,
	LumenChatAddMembers,
	LumenChatCastDeleteVote,
	LumenChatChannels,
	LumenChatCreateGroup,
	LumenChatDeleteMessage,
	LumenChatGetDeleteVote,
	LumenChatList,
	LumenChatMarkRead,
	LumenChatOpenDm,
	LumenChatPinMessage,
	LumenChatPost,
	LumenChatReactMessage,
	LumenCommunity,
...
const canCreateGroup = computed(
	() => groupName.value.trim().length > 0 && groupPick.value.length + 1 >= MIN_GROUP_MEMBERS,
)
...
const { avatarFor } = useLumenCommunityAvatars(avatarPeople)
...
function headForNick(nick: string): string {
	...
	if (!person) return Lumen_COMMUNITY_STEVE_HEAD
...
async function markChannelRead(channelId: number, maxId: number) {
	if (maxId <= 0) return
	try {
		await LumenChatMarkRead(channelId, maxId)
...
async function refreshChannels() {
	...
	const rows = await LumenChatChannels()
...
async function loadMessages(reset = false) {
	...
	const rows = await LumenChatList(channelId, reset ? 0 : lastId.value)
	if (reset) {
...
async function send() {
	...
	sending.value = true
	try {
		const posted = await LumenChatPost(selectedId.value, text)
...
async function openDm(userId: number) {
	try {
		const channel = await LumenChatOpenDm(userId)
		await refreshChannels()
...
async function createGroup() {
	...
	try {
		const channel = await LumenChatCreateGroup(name, groupPick.value)
		await refreshChannels()
		selectedId.value = channel.id
		groupName.value = ''
		groupPick.value = []
...
async function refreshDeleteVote() {
	...
	try {
		deleteVote.value = await LumenChatGetDeleteVote(selected.value.id)
	} catch {
		deleteVote.value = null
	}
}
...
async function castDeleteVote(yes: boolean) {
	...
	try {
		const result = await LumenChatCastDeleteVote(selected.value.id, yes)
...
async function inviteMembers() {
	...
	try {
		await LumenChatAddMembers(selectedId.value, invitePick.value)
		invitePick.value = []
		showInvite.value = false
		await refreshChannels()
		await refreshDeleteVote()
...
async function reactToMessage(message: ChatMessage, emoji: string) {
	try {
		const updated = await LumenChatReactMessage(message.id, emoji)
...
async function togglePin(message: ChatMessage) {
	try {
		await LumenChatPinMessage(message.id, !message.pinned)
...
async function deleteMessage(message: ChatMessage) {
	...
	try {
		await LumenChatDeleteMessage(message.id)
...
async function beginMrpackInstall(url: string) {
	...
	try {
		const path = await LumenCacheMrpackUrl(url)
...
const messages = defineMessages({
	heading: { id: 'Lumen.chat.heading', defaultMessage: 'Czat' },
	close: { id: 'Lumen.chat.close', defaultMessage: 'Zamknij czat' },
	placeholder: {
		id: 'Lumen.chat.placeholder',
		defaultMessage: 'Napisz wiadomosc...',
	},
	send: { id: 'Lumen.chat.send', defaultMessage: 'Wyslij' },
	installPack: { id: 'Lumen.chat.install-pack', defaultMessage: 'Zainstaluj paczke' },
	installConfirm: {
		id: 'Lumen.chat.install-confirm',
		defaultMessage: 'Zainstaluj',
	},
	installCancel: {
		id: 'Lumen.chat.install-cancel',
		defaultMessage: 'Anuluj',
	},
	installPreviewLoading: {
		id: 'Lumen.chat.install-preview-loading',
		defaultMessage: 'Ladowanie podgladu paczki...',
	},
	installPreviewTitle: {
		id: 'Lumen.chat.install-preview-title',
		defaultMessage: 'Zainstalowac {name}?',
	},
	installPreviewMeta: {
		id: 'Lumen.chat.install-preview-meta',
		defaultMessage: '{gameVersion} · {modloader}',
	},
	signInHint: {
		id: 'Lumen.chat.sign-in-hint',
		defaultMessage: 'Zaloguj sie do Lumen, aby korzystac z czatu.',
	},
	signInAction: { id: 'Lumen.chat.sign-in-action', defaultMessage: 'Zaloguj sie' },
	emptyChannels: {
		id: 'Lumen.chat.empty-channels',
		defaultMessage: 'Brak rozmow.',
	},
	emptyMessages: {
		id: 'Lumen.chat.empty',
		defaultMessage: 'Brak wiadomosci. Powiedz czenst lub wyslij link do paczki.',
	},
	newDm: { id: 'Lumen.chat.new-dm', defaultMessage: 'Napisz do kogos' },
	newGroup: { id: 'Lumen.chat.new-group', defaultMessage: 'Nowa grupa' },
	groupName: { id: 'Lumen.chat.group-name', defaultMessage: 'Nazwa grupy' },
	createGroup: { id: 'Lumen.chat.create-group', defaultMessage: 'Utworz grupe' },
	pickMembers: {
		id: 'Lumen.chat.pick-members',
		defaultMessage: 'Dodaj czlonkow',
	},
	conversations: {
		id: 'Lumen.chat.conversations',
		defaultMessage: 'Rozmowy',
	},
	selectConversation: {
		id: 'Lumen.chat.select-conversation',
		defaultMessage: 'Wybierz rozmowe',
	},
	messageDeleted: {
		id: 'Lumen.chat.message-deleted',
		defaultMessage: 'Wiadomosc usunieta',
	},
	imageAttachment: {
		id: 'Lumen.chat.image-attachment',
		defaultMessage: 'Obraz',
	},
	pin: { id: 'Lumen.chat.pin', defaultMessage: 'Przypnij' },
	unpin: { id: 'Lumen.chat.unpin', defaultMessage: 'Odepnij' },
	delete: { id: 'Lumen.chat.delete', defaultMessage: 'Usun' },
	pinnedBadge: { id: 'Lumen.chat.pinned', defaultMessage: 'Przypiety' },
	inviteMembers: {
		id: 'Lumen.chat.invite-members',
		defaultMessage: 'Zaprosc czlonkow',
	},
	inviteConfirm: {
		id: 'Lumen.chat.invite-confirm',
		defaultMessage: 'Dodaj do grupy',
	},
	groupMinMembers: {
		id: 'Lumen.chat.group-min-members',
		defaultMessage: 'Wybierz co najmniej 2 inne osoby (3 lacznie z Toba).',
	},
	deleteGroup: {
		id: 'Lumen.chat.delete-group',
		defaultMessage: 'Usun grupe',
	},
	deleteVoteTitle: {
		id: 'Lumen.chat.delete-vote-title',
		defaultMessage: 'Glosuj za usunieciem tej grupy',
	},
	deleteVoteHint: {
		id: 'Lumen.chat.delete-vote-hint',
		defaultMessage: '{yes} z {needed} glosow tak potrzebnych ({members} czlonkow).',
	},
	deleteVoteYes: {
		id: 'Lumen.chat.delete-vote-yes',
		defaultMessage: 'Glosuj tak',
	},
	deleteVoteNo: {
		id: 'Lumen.chat.delete-vote-no',
		defaultMessage: 'Glosuj nie',
	},
	deleteVoteStart: {
		id: 'Lumen.chat.delete-vote-start',
		defaultMessage: 'Rozpocznij glosowanie',
	},
	deleteVoteConfirm: {
		id: 'Lumen.chat.delete-vote-confirm',
		defaultMessage: 'Dwie trzecie glosow musi byc na tak, aby usunac grupe.',
	},
	unreadBadge: {
		id: 'Lumen.chat.unread',
		defaultMessage: '{count} nieprzeczytanych',
	},
	backToList: {
		id: 'Lumen.chat.back-to-list',
		defaultMessage: 'Powrot do rozmow',
	},
	messageActions: {
		id: 'Lumen.chat.message-actions',
		defaultMessage: 'Akcje wiadomosci',
	},
})
</script>
...
<template>
	<component
		...
		:class="
			embedded
				? 'Lumen-chat-embedded w-full min-h-0 flex-1 border-0'
				: 'Lumen-chat-rail w-[min(560px,52vw)] shrink-0 border-0 border-r border-solid border-surface-5 bg-bg-raised'
		"
	>
		<ContextMenu ref="messageOptions" :label="formatMessage(messages.messageActions)" />
		<div v-if="!embedded" class="Lumen-chat-rail__accent" aria-hidden="true" />
		<div
			v-if="!embedded"
			class="Lumen-chat-rail__header flex items-center justify-between gap-2 border-0 border-b border-solid border-surface-5 px-3 py-2"
		>
		...
		<template v-else>
			<div class="flex min-h-0 flex-1" :class="embedded ? 'flex-col' : 'flex-row'">
				<div
				...
				<div
					...
					:class="
						embedded ? 'bg-transparent' : 'bg-[color-mix(in_srgb,var(--color-bg)_70%,transparent)]'
					"
				>
					<div
