...
import { ExternalIcon, InfoIcon, TagCategoryFlagIcon } from '@lumen/assets'
import { IntlFormatted, useScrollIndicator, useVIntl } from '@lumen/ui'
...
const eventGroups = computed<EventGroup[]>(() => {
	const groups = mergeProjectGroupsForLumenEventDistance(
...
function mergeProjectGroupsForLumenEventDistance(groups: EventGroup[]): EventGroup[] {
	let nextGroups = groups

	while (true) {
		const result = mergeProjectGroupsForLumenEventDistanceOnce(nextGroups)
...
function mergeProjectGroupsForLumenEventDistanceOnce(groups: EventGroup[]) {
	const clusterLayouts = getCollisionClusterLayouts(groups)

	for (const layout of clusterLayouts) {
		const projectGroupsToMerge = getProjectGroupsToMergeForLumenEventDistance(layout)
...
function getProjectGroupsToMergeForLumenEventDistance(layout: CollisionClusterLayout) {
	...
	const overLimitPlacement = getMostDisplacedLumenPlacement(placements, distanceLimit)
	if (!overLimitPlacement) return []

	const displacedLumenPlacement = overLimitPlacement
	const offset = displacedLumenPlacement.markerX - displacedLumenPlacement.group.x
	const projectPlacementsOnOffsetSide = projectPlacements.filter((placement) =>
		offset > 0
			? placement.index < displacedLumenPlacement.index
			: placement.index > displacedLumenPlacement.index,
	)
...
function getMostDisplacedLumenPlacement(
	...
	for (const placement of placements) {
		if (!isLumenEventGroup(placement.group)) continue
...
function isLumenEventGroup(group: EventGroup) {
	return group.groupKey === 'Lumen'
}
