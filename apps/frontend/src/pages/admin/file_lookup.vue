	...
	<div>
		...
		<div class="card flex flex-col gap-3">
			...
			<template v-if="lookupResult">
				<h3 class="mb-0 text-lg font-extrabold text-contrast">Lumen project:</h3>
				<nuxt-link
				...
				<h3 class="mb-0 text-lg font-extrabold text-contrast">Lumen version:</h3>
				<nuxt-link
...
import { FileIcon, SpinnerIcon, UploadIcon } from '@lumen/assets'
import {
	Admonition,
	Avatar,
	CopyCode,
...
import type { Project, Version } from '@lumen/utils'
...
async function lookupFile(hash: string): Promise<void> {
	...
	try {
		...
		if (error.status === 404) {
			lookupError.value = `File not found on Lumen across projects you have access to.`
		} else {
