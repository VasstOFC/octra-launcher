# apps/frontend â€” Modrinth Website

Nuxt 3 application serving the main Modrinth website. Uses Vue 3, Tailwind CSS v3, and file-based routing.

## Architecture

Nuxt 3 with SSR â€” pages are server-rendered and hydrated on the client. Uses `$fetch` for server-side data fetching and `@lumen/api-client` (via `NuxtModrinthClient`) for client-side API calls.
...
Use `@lumen/api-client` via `injectLumenClient()` for all API calls. See `packages/api-client/AGENTS.md` for the full API client documentation.
