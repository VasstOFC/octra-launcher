...
{
	let info = crate::api::pack::featured::featured_pack_info().await;
	...
	create_modpack_instance(
		...
		Some(InstallPostInstallEdit {
			...
			link: Some(InstanceLink::ImportedModpack {
				...
				version_number: Some(crate::nervia::FEATURED_PACK_VERSION.to_string()),
				filename: Some(crate::nervia::FEATURED_PACK_CACHE_NAME.to_string()),
			}),
		}),
	)
	.await
