...
pub async fn publish_to_registry(
	...
	let bearer = crate::lumen_accounts::bearer_token().await;
	let api_key = nervia::skins_api_key();
	if bearer.is_none() && api_key.is_none() {
		tracing::warn!(
			"Octra skins: no Octra session and no {env}; skipping publish for {name}",
			env = nervia::SKINS_API_KEY_ENV,
		);
		return false;
	}
	...
	let send = |method: reqwest::Method| {
		...
		if let Some(token) = bearer.as_deref() {
			...
			request = request.header("X-Octra-Key", key);
		}
		request.send()
	};
