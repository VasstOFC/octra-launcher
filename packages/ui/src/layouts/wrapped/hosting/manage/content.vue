...
import { type Archon, type Labrinth, LumenApiError } from '@lumen/api-client'
import { ClipboardCopyIcon } from '@lumen/assets'
...
const props = withDefaults(
	...
	{
		ownerAvatarUrlBase: 'https://Lumen.com',
	},
)
...
const currentModpackVersionId = computed(() => {
	const spec = contentQuery.data.value?.modpack?.spec
	return spec?.platform === 'Lumen' ? spec.version_id : null
})
...
const managedContent = computed<ManagedContentData | null>(() => {
	...
	return {
		card: {
			...
			versionLink:
				projectId && mp.spec.platform === 'Lumen'
...
function handleUploadFiles() {
	...
	input.onchange = async () => {
		...
		try {
			const fileRecognition = await Promise.all(files.map(isFileOnLumen))
...
async function isFileOnLumen(file: File) {
	...
	try {
		...
		return !(error instanceof LumenApiError && error.statusCode === 404)
	}
}
...
async function handleModpackUpdate() {
	if (setupActionDisabled.value) return
	const mp = contentQuery.data.value?.modpack
	if (!mp || mp.spec.platform !== 'Lumen') return
...
function handleModalUpdate(selectedVersion: Labrinth.Versions.v2.Version, event?: MouseEvent) {
	if (updatingModpack.value) {
		...
		const currentVersionId = mpSpec?.platform === 'Lumen' ? mpSpec.version_id : undefined
...
provideContentManager({
	...
	mapToTableItem: (item) => {
		...
		const addon = addonLookup.value.get(item.file_name)
		const hasLumenProject = !!addon?.project_id || (!!item.installing && !!item.project?.id)
		const projectSlugOrId = item.project.slug ?? item.project.id
		return {
			id: getContentItemId(item),
			project: item.project,
			projectLink: hasLumenProject ? `/${projectType}/${projectSlugOrId}` : undefined,
			version: item.version,
			versionLink:
				hasLumenProject && item.version?.id
					? `/${projectType}/${projectSlugOrId}/version/${item.version.id}`
					: undefined,
			owner: item.owner
				? { ...item.owner, link: item.owner.link ?? `/${item.owner.type}/${item.owner.id}` }
				: undefined,
			external: item.external ?? !hasLumenProject,
			enabled: item.enabled,
		}
	},
		...
		<ContentPageLayout :bottom-padding="false">
			<template #modals>
				...
				<ContentUpdaterModal
					...
					:current-version-id="
						updatingModpack
							? contentQuery.data.value?.modpack?.spec.platform === 'Lumen'
