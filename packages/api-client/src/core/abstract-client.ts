...
import { LumenApiError, LumenServerError } from './errors'
...
/**
 * Abstract base client for Lumen APIs
 */
export abstract class AbstractLumenClient extends AbstractUploadClient {
	 ...
	 * @throws {LumenApiError} When the request fails or features throw errors
	 */
	...
	protected buildUrl(path: string, baseUrl: string, version: number | 'internal' | string): string {
		...
		if (version === 'internal') {
			...
			// Custom version string (e.g., 'v0', 'Lumen/v0')
			versionPath = `/${version}`
		}
	...
	protected attachArchonSentryCaptureHeader(options: RequestOptions): void {
		...
		options.headers['Lumen-sentry-capture'] = '1'
	}
	...
	/**
	 * Normalize an error into a LumenApiError
	 *
	 ...
	 */
	protected normalizeError(error: unknown, context?: RequestContext): LumenApiError {
		if (error instanceof LumenApiError) {
			return error
		}

		return LumenApiError.fromUnknown(error, context?.path)
	}
	...
	protected createNormalizedError(
		...
		if (statusCode && responseData) {
			return LumenServerError.fromResponse(statusCode, responseData)
		}
