import { AbstractLumenClient } from '../core/abstract-client'
import { LumenApiError } from '../core/errors'
 ...
 *
 * Platform-specific clients should extend this instead of AbstractLumenClient
 * to inherit the XHR upload implementation.
 */
export abstract class XHRUploadClient extends AbstractLumenClient {
		...
		const handle: UploadHandle<T> = {
			promise: (async () => {
				...
				if (abortController.signal.aborted) {
					throw new LumenApiError('Upload cancelled')
				}
	...
	protected executeXHRUpload<T>(
		...
		return new Promise<T>((resolve, reject) => {
			...
			xhr.addEventListener('error', () => reject(new LumenApiError('Upload failed')))
			xhr.addEventListener('abort', () => reject(new LumenApiError('Upload cancelled')))
			xhr.addEventListener('timeout', () => reject(new LumenApiError('Upload timed out')))
	...
	protected createUploadError(xhr: XMLHttpRequest): LumenApiError {
