 ...
 *
 * const client = new NuxtLumenClient({
 ...
 * ```
 */
export class NuxtLumenClient extends XHRUploadClient {
	...
	upload<T = void>(path: string, options: UploadRequestOptions): UploadHandle<T> {
		// @ts-expect-error - import.meta is provided by Nuxt
		if (import.meta.server) {
			throw new LumenApiError('upload() is not supported during SSR')
		}
	...
	protected async executeStreamRequest(
		...
		try {
			...
			if (!response.body) {
				throw new LumenApiError('Streaming response has no readable body', {
